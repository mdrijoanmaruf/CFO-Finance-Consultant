...
export default function NewInsightPage() {
  ...
  const handleContentChange = (html: string) => {
    if (activeLang === "en") {
      set("content", html);
      set("readingTime", estimateReadTime(html));
    } else {
      set("contentBn", html);
    }
  };
  ...
  const handleSave = async (publishNow?: boolean) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast.success(publishNow ? "Insight published!" : "Insight saved as draft");
        router.push("/admin/insights");
      } else {
        toast.error(data.error || "Failed to save insight");
      }
    } catch {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[13px] text-white/50 mt-1 tracking-wide">
            Create a new insight article for your audience
          </p>
        </div>
        <Link
          href="/admin/insights"
        ...
        >
          <FiArrowLeft className="w-4 h-4" /> Back to Insights
        </Link>
      </div>

      {/* Form grid — fully transparent cards */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Main column */}
        <div className="lg:col-span-2 space-y-6">
          {/* Language Tabs */}
          <div className="flex bg-white/[0.03] rounded-xl border border-white/10 p-1.5 gap-1">
            <button
              type="button"
          ...
          {/* Basic Info */}
          <div className="rounded-xl p-5 border border-white/[0.06] bg-black/10 backdrop-blur-sm">
            <SectionHeader
              icon={<FiInfo className="w-4 h-4" />}
              iconClass="bg-[#c8a96e]/20 text-[#c8a96e]"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="md:col-span-2">
                ...
                <input
                  type="text"
                  value={activeLang === "en" ? form.title : form.titleBn}
                  onChange={(e) => handleTitleChange(e.target.value)}
                  placeholder={
                    activeLang === "en" ? "Insight title" : "ইনসাইটের শিরোনাম"
                  }
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <input
                      ...
                      className="flex-1 px-4 py-2.5 text-sm bg-white/5 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-[#c8a96e] focus:border-transparent outline-none font-mono transition-all placeholder:text-white/20"
                    />
                  </div>
                </div>
              )}
              ...
              <div>
                <label className="block text-xs font-semibold text-white/50 uppercase tracking-wider mb-2">
                  ...
                  <span className="text-white/30 font-normal normal-case ml-1">
                    (
                    {(activeLang === "en" ? form.excerpt : form.excerptBn).length}
                    /300)
                  </span>
                </label>
                <textarea
                  value={activeLang === "en" ? form.excerpt : form.excerptBn}
                  onChange={(e) =>
                    set(activeLang === "en" ? "excerpt" : "excerptBn", e.target.value)
                  }
                  placeholder={
                    activeLang === "en"
                      ? "Brief summary shown in insight cards..."
                      : "সংক্ষিপ্ত বিবরণ..."
                  }
                  rows={3}
                  maxLength={300}
                  className="w-full px-4 py-2.5 text-sm bg-white/5 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-[#c8a96e] focus:border-transparent outline-none resize-none transition-all placeholder:text-white/20 custom-scrollbar-gold"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-white/50 uppercase tracking-wider mb-2">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl p-5 border border-white/[0.06] bg-black/10 backdrop-blur-sm">
            ...
            {form.coverImage ? (
              ...
              >
                {coverUploading ? (
                  <FiLoader className="w-8 h-8 text-[#c8a96e] animate-spin mb-2" />
                ) : (
                  <>
            ...
            {form.tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {form.tags.map((tag) => (
                  <span
                    key={tag}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white/5 text-white/80 text-[11px] font-semibold tracking-wide uppercase rounded-full border border-white/10"
                  >
                    {tag}
                    <button
                      type="button"
                      onClick={() => removeTag(tag)}
                      className="hover:text-[#c8a96e] text-white/40 transition-colors"
                    >
                ...
                <label key={key} className="flex items-start gap-3 cursor-pointer">
                  <div
                    className={`w-10 h-5 rounded-full transition-all relative shrink-0 mt-0.5 ${
                      form[key as keyof typeof form] ? "bg-[#c8a96e]" : "bg-white/10"
                    }`}
          ...
          {/* SEO */}
          <div className="rounded-xl p-5 border border-white/[0.06] bg-black/10 backdrop-blur-sm space-y-4">
            <SectionHeader
              icon={<FiSettings className="w-4 h-4" />}
              iconClass="bg-[#c8a96e]/20 text-[#c8a96e]"
              label="SEO Settings"
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-2 text-sm bg-white/5 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-[#c8a96e] focus:border-transparent outline-none resize-none transition-all placeholder:text-white/20 custom-scrollbar-gold"
              />
            </div>
          </div>
          ...
          <div className="rounded-xl p-5 border border-white/[0.06] bg-black/10 backdrop-blur-sm">
            ...
            <input
              ...
              value={form.views}
              onChange={(e) =>
                set("views", e.target.value === "" ? "" : parseInt(e.target.value) || 0)
              }
              placeholder="0"
              className="w-full px-4 py-2 text-sm bg-white/5 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-[#c8a96e] focus:border-transparent outline-none transition-all placeholder:text-white/20"
            />
            <p className="text-[11px] text-white/40 mt-2 leading-relaxed">
              Initial view count. Increments automatically on each visit.
            </p>
          </div>
          ...
          <div className="rounded-xl p-5 border border-white/[0.06] bg-black/10 backdrop-blur-sm">
            ...
            <input
              ...
              value={form.order}
              onChange={(e) =>
                set("order", e.target.value === "" ? "" : parseInt(e.target.value) || 0)
              }
              placeholder="0"
              className="w-full px-4 py-2 text-sm bg-white/5 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-[#c8a96e] focus:border-transparent outline-none transition-all placeholder:text-white/20"
            />
            <p className="text-[11px] text-white/40 mt-2 leading-relaxed">
              Lower numbers appear first. Used for custom sorting.
            </p>
          </div>
        </div>
      </div>
